...
def checkBundledPO(po):
    if po.get("lines", None) is None:
        raise PONoLineError("PO has no Line information. It might be a DBE shell with no corresponding PO.")
    reqs = set([line["Requisition_Data"]["Req_ID"] for line in po["lines"] if line.get("Requisition_Data", None)])
    if len(reqs) > 1:
        raise POBundledError("{} REQs found with {}.".format(len(reqs), po["PO_No"]))
...
def getReqWorklistEvents(req_worklists):
    results = []
    req_worklists = sorted(req_worklists, key=lambda k: (k["Appr_Inst"], k["Approval_Number"], k["Date_Time"]))
    seen_timestamps = set()
    ...
    for req_wl in req_worklists:
        ...
        if req_wl["Appr_Stat"] == "I":
            ...
            text = "{}, {}".format(req_wl["Work_List"], req_wl["Approval_Number"])
...
def getPOWorklistEvents(po_worklists):
    results = []
    po_worklists = sorted(po_worklists, key=lambda k: (k["Appr_Inst"], k["Approval_Number"], k["Event_Date_Time"]))
    seen_timestamps = set()
...
for loc in writelocation:
    ...
    for req in tqdm(db.REQ_DATA.find()):
        ...
        if len(pos) > 0:
            timeline_events = [*req_evnets, *po_events, *req_worklist_events, *po_worklist_events]
