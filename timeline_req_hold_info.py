...
df = lake.get_df("timeline_req_hold_info",
                 "PLIT_TIMELINE_REQ_HOLD_INFO", datestring)
...
for db_name in db_names:
    ...
    req_id_tuples = zip(df["Business_Unit"].tolist(),
                        df["Requisition_ID"].tolist())
    seen_req_dict = dict(((b_unit, req_id), False)
                         for b_unit, req_id in req_id_tuples)
